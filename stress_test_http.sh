...
# Tests walker execution via CLI under concurrent load
...
echo "Total time:"
awk -F',' 'NR>1 {sum+=$NF; count++} END {print sum "ms (" count " runs)"}' "$RESULTS_DIR/results.csv"
echo "Average time per walker:"
awk -F',' 'NR>1 {sum+=$NF; count++} END {print sum/count "ms"}' "$RESULTS_DIR/results.csv"
