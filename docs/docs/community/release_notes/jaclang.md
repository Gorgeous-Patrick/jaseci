...
- **Typed Traversal Graph (TTG) Generation**: Added a production-ready TTG system that statically analyzes walker archetypes to extract typed traversal constraints and generates prefetch hints at runtime. Includes `JacTTGPass` for static analysis, `JacTTGGenerator` for runtime BFS expansion, and `Root.graph`/`Root.type_map` for topology tracking using type-safe `issubclass` checks instead of string comparison.
