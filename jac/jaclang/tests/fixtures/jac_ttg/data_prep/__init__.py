...
def get_subgraph(extracted_file, subgraph_file, target_size=1000, limit=1000000, max_degree=100):
    if os.path.exists(subgraph_file):
        print(f"Subgraph already exists at {subgraph_file}. Loading...")
        return nx.read_edgelist(subgraph_file, create_using=nx.MultiDiGraph(), data=False)
    else:
