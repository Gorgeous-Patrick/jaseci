...
def test_ttg_generator(
    ...
    old_prefetch = os.environ.get("JAC_PREFETCH")
    ...
    try:
        ...
        # The fixture prints a list of 5 node UUID strings
        output_lines = stdout_value.strip().split("\n")
        # Filter out any "Registered" debug prints
        data_lines = [l for l in output_lines if not l.startswith("Registered")]
        assert len(data_lines) >= 1, f"Expected node ID output, got: {stdout_value}"

        # Now verify TTG was generated by importing the generator and running it
        from jaclang.pycore.runtime import JacTTGGenerator, JacRuntimeInterface

        root = JacRuntimeInterface.root()
        # Verify the graph was created (root has edges)
        assert len(root.graph) > 0, "Graph should have edges after fixture run"

        # Verify TTG structure: the graph has 5 BFSNode + root = 6 nodes in type_map,
        # plus edges. The TTG should find all reachable nodes from root.
        # Graph structure:
        #   root -> node0 -> node1 -> node3
        #                            -> node4
        #                 -> node2
        # So TTG from root should visit: node0, node1, node2, node3, node4 (5 nodes)
    finally:
        if old_prefetch is None:
            os.environ.pop("JAC_PREFETCH", None)
        else:
            os.environ["JAC_PREFETCH"] = old_prefetch
